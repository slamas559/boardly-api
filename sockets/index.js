  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-room", async (data) => {
      try {
        ...
        const existingConnection = activeUsers.get(user.id);
        if (existingConnection && existingConnection.socketId !== socket.id) {
          ...
          if (existingSocket) {
            existingSocket.emit("force-disconnect", {
              reason: "Another session detected",
              message: "Your session was ended because you joined from another device/browser."
            });
            // Clean up the existing connection
            const existingRoomId = existingConnection.roomId;
            existingSocket.leave(existingRoomId);
            removeUserFromRoom(existingConnection.socketId, existingRoomId);
            joinedRooms.delete(existingConnection.socketId);
            
            existingSocket.disconnect();
            console.log("user forcefully disconnected!!")

            if (existingRoomId) {
              broadcastRoomStats(io, existingRoomId);
            }
          }
        }
